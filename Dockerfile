...
ENV			POWERSHELL_VERSION v6.0.0-alpha.10
ENV 			TERM xterm
...
RUN 			apt-get update && \
				apt-get install -y git subversion nano wget curl iputils-ping dnsutils sudo
...
#Powershell
RUN			bash <(curl -fsSL https://raw.githubusercontent.com/PowerShell/PowerShell/$POWERSHELL_VERSION/tools/download.sh)
